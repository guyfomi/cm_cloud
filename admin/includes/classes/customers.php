...
class osC_Customers_Admin {
	function getData($id, $key = null) {
		...
		$Qcustomer = $osC_Database->query ( 'select c.*, ab.entry_gender as customers_gender,ab.entry_street_address as street_address,ab.entry_postcode as postcode,ab.entry_city as city,ab.entry_country_id as country_id,ab.entry_zone_id as zone_id,ab.entry_fax as fax_number from :table_customers c left join :table_address_book ab on (c.customers_id = ab.customers_id) where c.customers_id = :customers_id' );
	...
	function save($id = null, $data, $send_email = false) {
        ...
        if (is_numeric($id)) {
            $Qcustomer = $osC_Database->query('update :table_customers set customers_gender = :customers_gender, customers_firstname = :customers_firstname, customers_lastname = :customers_lastname, customers_email_address = :customers_email_address, customers_dob = :customers_dob, customers_newsletter = :customers_newsletter, customers_groups_id = :customers_groups_id,customers_telephone = :customers_telephone, customers_status = :customers_status, date_account_last_modified = :date_account_last_modified,roles_id = :roles_id,administrators_id = :administrators_id,customers_surname = :customers_surname where customers_id = :customers_id');
            $Qcustomer->bindRaw(':date_account_last_modified', 'now()');
            $Qcustomer->bindInt(':customers_id', $id);
        } else {
            $Qcustomer = $osC_Database->query('insert into :table_customers (customers_gender, customers_firstname, customers_lastname, customers_email_address, customers_dob, customers_newsletter, customers_status, customers_groups_id, number_of_logons, date_account_created,customers_telephone,roles_id,administrators_id,customers_surname) values (:customers_gender, :customers_firstname, :customers_lastname, :customers_email_address, :customers_dob, :customers_newsletter, :customers_status, :customers_groups_id, :number_of_logons, :date_account_created,:customers_telephone,:roles_id,:administrators_id,:customers_surname)');
        ...
        $Qcustomer->bindValue(':customers_surname', $data['customers_surname']);
        ...
        $Qcustomer->bindInt(':roles_id', $data['roles_id']);
        $Qcustomer->bindInt(':administrators_id', $data['administrators_id']);
		...
		if ($osC_Database->isError ()) {
            ...
            if (is_numeric($id)) {
                $event['description'] = "Impossible de modifier le compte du client " . $data['customers_surname'] . " Raison : " . $osC_Database->getError();
            } else {
                $event['description'] = "Impossible de creer le compte du client " . $data['customers_surname'] . " Raison : " . $osC_Database->getError();
            }
        ...
        $Qab->bindValue(':entry_company', $data['customers_surname']);
        ...
        if ($osC_Database->isError()) {
            ...
            if (is_numeric($id)) {
                $event['description'] = "Impossible de modifier le carnet d'adresse du client " . $data['customers_surname'] . " Raison : " . $osC_Database->getError();
            } else {
                $event['description'] = "Impossible de creer le carnet d'adresse du client " . $data['customers_surname'] . " Raison : " . $osC_Database->getError();
            }
		...
		if ($error === false) {
            ...
            if (is_numeric($id)) {
                $event['description'] = "Le compte du client " . $data['customers_surname'] . " a ete modifie par " . $_SESSION['admin']['username'];
            } else {
                $event['description'] = "Le compte du client " . $data['customers_surname'] . " a ete cree par " . $_SESSION['admin']['username'];
            }
	...
	function delete($id, $delete_reviews = true) {
        ...
        if ($error === false) {
            $Qcustomers = $osC_Database->query ( 'delete from :table_customers_to_groups where customers_id = :customers_id' );
