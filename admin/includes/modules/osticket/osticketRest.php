...
class osticketRest {
    ...
    public function post($url, Array $arg = null) {
        ...
        if ($head['http_code'] >= 400) {
            throw new osticketException("{$this->getErrorMsg($head['http_code'])} on POST request ({$url})", $head['http_code'], $body);
        }
    ...
    public function put($url, $fileString, $isBinary = false) {
        try {
            ...
            if ($streamBuffer === false) {
                throw new osticketException('failed to open PHP stream buffer');
            }
        ...
        if ($head['http_code'] >= 400) {
            throw new osticketException("{$this->getErrorMsg($head['http_code'])} on multipart PUT request ({$url})", $head['http_code'], $body);
        }
    ...
    public function multiput($url, $resourceDescriptorXml, $fileUri, $fileContent, $isBinary = true) {
        ...
        if ($head['http_code'] >= 400) {
            throw new osticketException("{$this->getErrorMsg($head['http_code'])} on PUT request ({$url})", $head['http_code'], $body);
        }
    ...
    public function delete($url) {
        ...
        if ($head['http_code'] >= 400) {
            throw new osticketException("{$this->getErrorMsg($head['http_code'])} on DELETE request ({$url})", $head['http_code'], $body);
        }
