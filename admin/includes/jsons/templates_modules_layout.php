...
class toC_Json_Templates_Modules_Layout
{
    ...
    function listTemplatesModulesLayout()
    {
        ...
        $Qlayout = $osC_Database->query('select b2p.*, b.title as box_title from :table_templates_boxes_to_pages b2p, :table_templates_boxes b where b2p.templates_id = :templates_id and b2p.templates_boxes_id = b.id and b.modules_group = :modules_group order by b2p.content_page, b2p.boxes_group, b2p.sort_order, b.title');
        ...
        while ($Qlayout->next()) {
            $records[] = array('id' => $Qlayout->ValueInt('id'),
                               ...
                               'categories_name' => $Qlayout->Value('categories_name'),
    ...
    function getPages($include_custom_pages = true)
    {
        global $toC_Json, $osC_Database;
        ...
        if($include_custom_pages == true)
        {
            $osC_CategoryTree = new osC_CategoryTree_Admin();

            foreach ($osC_CategoryTree->getTree() as $value) {
                if ($value['id'] == -1) {
                    $entry = array('id' => $filter_id . '/index/index',
                                   'text' => $value['title']);
                }
                else
                {
                    $entry = array('id' => $filter_id . '/index/cPath=' . $value['id'],
                                   'text' => $value['title']);
                }
    ...
    function saveBoxLayout()
    {
        ...
        $data = array('box' => isset($_REQUEST['box']) ? $_REQUEST['box'] : '',
                      ...
                      'page_specific' => (isset($_REQUEST['page_specific']) && ($_REQUEST['page_specific'] == 'on')
                              ? true : false),
