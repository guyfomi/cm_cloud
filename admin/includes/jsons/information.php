...
*/
  ...
  class toC_Json_Information {
    ...
    function loadArticle() {
      ...
      $Qad = $osC_Database->query('select content_name, articles_url, articles_description, articles_page_title, articles_meta_keywords, articles_meta_description, language_id from :table_articles_description where articles_id = :articles_id');
      $Qad->bindTable(':table_articles_description', TABLE_ARTICLES_DESCRIPTION);
      $Qad->bindInt(':articles_id', $_REQUEST['articles_id']);
      $Qad->execute();
      
      while ($Qad->next()) {
        $data['content_name[' . $Qad->valueInt('language_id') . ']'] = $Qad->value('content_name');
