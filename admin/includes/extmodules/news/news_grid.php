...
Toc.news.NewsGrid = function(config) {
  var that = this;
  config = config || {};
  ...
  config.cm = new Ext.grid.ColumnModel([
    ...
    { header: 'Date creation', align: 'left', dataIndex: 'date_created', sortable: true},
    config.rowActions
  ]);
  ...
  Toc.news.NewsGrid.superclass.constructor.call(this, config);
};

Ext.extend(Toc.news.NewsGrid, Ext.grid.GridPanel, {
  
  onAdd: function() {
    var dlg = this.owner.createNewsDialog();
  ...
  onEdit: function(record) {
    ...
    dlg.show(record.get("news_id"),path);
  },
  
  onDelete: function(record) {
    var newsId = record.get('news_id');
  ...
  onClick: function(e, target) {
    ...
    if (row !== false) {
      ...
      if (action != 'img-button') {
        var newsId = this.getStore().getAt(row).get('news_id');
        ...
        switch(action) {
          ...
          case 'status-on':
            ...
            this.onAction(module, newsId, flag);
  ...
  onAction: function(action, newsId, flag) {
    Ext.Ajax.request({
      url: Toc.CONF.CONN_URL,
      params: {
        module: 'news',
      ...
      callback: function(options, success, response) {
        ...
        if (result.success == true) {
          var store = this.getStore();
          store.getById(newsId).set('content_status', flag);
          store.commitChanges();
