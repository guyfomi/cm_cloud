...
/* Class constructor */
    ...
    function initialize() {
      global $osC_Database, $osC_Language, $osC_Currencies, $osC_Image, $current_category_id;

      if ($current_category_id < 1) {
        $Qnewproducts = $osC_Database->query('select p.products_id,p.products_tax_class_id, p.products_price,pd.products_short_description, pd.products_name, pd.products_keyword, i.image from :table_products p left join :table_products_images i on (p.products_id = i.products_id and i.default_flag = :default_flag), :table_products_description pd where p.products_status = 1 and p.products_id = pd.products_id and pd.language_id = :language_id order by p.products_date_added desc limit 15');
      } else {
        $Qnewproducts = $osC_Database->query('select distinct p.products_id,p.products_tax_class_id, p.products_price,pd.products_short_description, pd.products_name, pd.products_keyword, i.image from :table_products p left join :table_products_images i on (p.products_id = i.products_id and i.default_flag = :default_flag), :table_products_description pd, :table_products_to_categories p2c, :table_categories c where c.parent_id = :parent_id and c.categories_id = p2c.categories_id and p2c.products_id = p.products_id and p.products_status = 1 and p.products_id = pd.products_id and pd.language_id = :language_id order by p.products_date_added desc limit 15');
      ...
      if ($Qnewproducts->numberOfRows()) {
        ...
        while ($Qnewproducts->next()) {
          ...
          $this->_content .= '<div style="margin-top: 10px; float:left; width: 25%; text-align: center">' .
                             '<p><strong>' . osc_link_object(osc_href_link(FILENAME_PRODUCTS, $Qnewproducts->value('products_id')), $Qnewproducts->value('products_name')) . '</strong></p>' .
//                             '<span style="display:block; height: 32px; text-align: center">' . osc_link_object(osc_href_link(FILENAME_PRODUCTS, $Qnewproducts->value('products_id')), $Qnewproducts->value('products_name')) . '</span>' .
                             osc_link_object(osc_href_link(FILENAME_PRODUCTS, $Qnewproducts->value('products_id')), $osC_Image->show($Qnewproducts->value('image'), $Qnewproducts->value('products_name'),'', 'thumbnails'), 'id="productImage' . $Qnewproducts->value('products_id') . '"') .
                             '<span style="color:red;font-weight:bold;display:block; padding: 3px; text-align: center">' . $product->getPriceFormated(true) . '</span><p><em><strong> ' . $Qnewproducts->value('products_short_description') . '</strong></em></p></div>';
...
//        $this->_content .= '<div class="clr"></div>';
      }
