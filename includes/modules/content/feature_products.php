      ...
      if ($Qproducts->numberOfRows()) {
        ...
        while ($Qproducts->next()) {
          ...
          $this->_content .= '<span style="width: 32%; float: left; text-align: center; padding: 2px;">' .
                             osc_link_object(osc_href_link(FILENAME_PRODUCTS, $Qproducts->value('products_id')), $osC_Image->show($Qproducts->value('image'), $Qproducts->value('products_name'),'','home')) . '<br />' .
                             osc_link_object(osc_href_link(FILENAME_PRODUCTS, $Qproducts->value('products_id')), $Qproducts->value('products_name')) . '<br />' .
                             '<span style = "color:red;font-weight:bold;display:block; padding: 3px; text-align: center" > ' . $osC_Product->getPriceFormated(true) . '</span >' .
                             '</span>';
