...
*/
    ...
    class osC_Content_articles_images extends osC_Modules
    {
        var $_title,
        $_code = 'articles_images',
...
/* Class constructor */
        ...
        function osC_Content_articles_images()
        {
            $this->_title = 'articles_images';
        }

        function initialize()
        {
            if (isset($_REQUEST['articles_id']) && is_numeric($_REQUEST['articles_id'])) {
                global $osC_Database;

                $Qimages = $osC_Database->query('select id, image from :table_content_images where content_id = :content_id and content_type = "articles" order by sort_order');
                $Qimages->bindTable(':table_content_images', TABLE_CONTENT_IMAGES);
                $Qimages->bindInt(':content_id', $_REQUEST['articles_id']);
                $Qimages->execute();
