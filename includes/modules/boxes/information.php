...
*/
  ...
  class osC_Boxes_information extends osC_Modules {
    ...
    function initialize() {
      ...
      $Qarticles = $osC_Database->query('select ad.articles_id, ad.content_name from :table_articles a, :table_articles_description ad where a.articles_id = ad.articles_id and ad.language_id = :language_id and a.articles_status = 1 and a.articles_categories_id = 1 order by a.articles_order');
      ...
      while ($Qarticles->next()) {
        $this->_content .= '<li>' . osc_link_object(osc_href_link(FILENAME_INFO, 'articles&articles_id='.$Qarticles->value('articles_id')), $Qarticles->value('content_name')) . '</li>';
      }
