<?php
    ...
    class osC_LayoutTree
    {
        ...
        function osC_LayoutTree($load_from_database = true)
        {
            ...
            if ($load_from_database === true) {
                ...
                if(isset($id))
                {
                    //$Qcategories = $osC_Database->query("select c.customers_id as categories_id,c.customers_status as categories_status, 0 as parent_id,' ' as categories_image,'customer' as content_type, c.customers_surname as categories_name,'' as categories_url,'' as categories_page_title,'' as categories_meta_keywords,'' as categories_meta_description,(select status from :table_status where content_type = 'customer' and content_id = c.customers_id) as monitoring_status from :table_customers c where c.administrators_id = :administrators_id order by c.customers_surname");
                    $Qcategories = $osC_Database->query("select c.customers_id as categories_id,c.administrators_id,c.customers_status as categories_status, 0 as parent_id,' ' as categories_image,'customer' as content_type, c.customers_surname as categories_name,'' as categories_url,'' as categories_page_title,'' as categories_meta_keywords,'' as categories_meta_description,(select status from :table_status where content_type = 'customer' and content_id = c.customers_id) as monitoring_status from :table_customers c order by c.customers_surname");
                    ...
                    while ($Qcategories->next()) {
                        $this->data[$Qcategories->valueInt('parent_id')][$Qcategories->valueInt('categories_id')] = array('categories_id' => $Qcategories->valueInt('categories_id'), 'name' => $Qcategories->value('categories_name'), 'categories_status' => $Qcategories->value('categories_status'), 'url' => $Qcategories->value('categories_url'), 'page_title' => $Qcategories->value('categories_page_title'), 'meta_keywords' => $Qcategories->value('categories_meta_keywords'), 'meta_description' => $Qcategories->value('categories_meta_description'), 'image' => $Qcategories->value('categories_image'),'monitoring_status' => $Qcategories->value('monitoring_status'),'content_type' => $Qcategories->value('content_type'), 'count' => 0);
                    }
        ...
        function buildExtJsonTreeArrayForUser($parent_id = 0, $tree_node_cls = 'x-tree-node-collapsed', $include_templates_pages = false, $show_home = true, $check_permissions = false)
        {
            ...
            if (isset($this->data[$parent_id])) {
                ...
                foreach ($this->data[$parent_id] as $category_id => $category) {
                    ...
                    $status = isset($category['monitoring_status']) && !empty($category['monitoring_status']) ? $category['monitoring_status'] : 'error';

                    $data['icon'] = 'templates/default/images/icons/16x16/' . strtolower($category['content_type']) . '_' . strtolower($status) . '.png';
     ...
     */
    class toC_LayoutTree extends osC_LayoutTree
    {
        var $leading_string = '';
