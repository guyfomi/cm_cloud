...
class osTicket_Database
{
    ...
    function &connect($server, $username, $password, $type = DB_DATABASE_CLASS)
    {
        require('database/mysql1.php');

        $class = 'osTicket_Database_' . $type;
        $object = new $class($server, $username, $password);
...
class osC_Database_Result
{
    ...
    function valueMixed($column, $type = 'string')
    {
        ...
        switch ($type) {
            case 'protected':
                return isset($this->result[$column]) ? osc_output_string_protected($this->result[$column]) : '';
                break;
            case 'int':
                return isset($this->result[$column]) ? (int)$this->result[$column] : 0;
                break;
            case 'decimal':
                return isset($this->result[$column]) ? (float)$this->result[$column] : 0;
                break;
            case 'string':
            default:
                return isset($this->result[$column]) ? $this->result[$column] : '';
        }
    }
